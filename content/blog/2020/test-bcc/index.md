...
## 前言
昨天在公司的服务网格月度讨论会上，一位同事为大家分享了在服务网格中使用 ebpf 来优化提升 istio 中 sidecar 和 RS 间的通信效率。这位同事在这方面做的还是比较深入的，而且在这块给内核和 istio 中提交了pr。有兴趣的同学可以看看他的 github:[https://github.com/ChenLingPeng](https://github.com/ChenLingPeng)。
...
今天下午和同事们开迭代评审会，因为相关我的事情不多，所以又想测试搞起一把。也刚好是年前买了一个台腾讯云的主机，一直没有怎么使用，今天看了，可以升级操作系统到 ubuntu 18.04，18.04 的内核模式就是 4.15 了，基本可以玩玩，不过安装好之后发现可以直接升级内核到 5.3，到 5.3 就有很多函数功能了。一下面先做了一些流水帐，记录了一下操作过程。

## ebpf 简单介绍
这里介绍也主要是从昨天分享得到，虽然之前也看过，但是毕竟没有使用过这玩意，所以认知还是不够深入。另外从今天下午[建盛大哥](http://opensourceway.community/)组织的技术写作分享课上学到的 learning by doing。所以这东西不做，理解始终是不够深入的。今天就实际操刀做了一遍。
...
这里面编程更重要的是后面的 3 点：Map types ，Program Types，helper functions。
#### Map types
...
在腾讯云可以自助重新安装操作系统，我根据内核版本和自己的熟悉程度，选择了 ubuntu 18.04 的镜像。重新安装非常快。
...
```
目前腾讯的源中的内核版本还是比较多的，最好的是 5.3.0， 所以我就选了这个最高版本。
