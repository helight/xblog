...
Sudoku.prototype = {
	...
	gameEvent : function() {
		...
		$("#gameBoard").click(function(eventObj){
			...
			var selectBoardPos = {top:posi.top + 50,left:posi.left + 50};
			if($target.hasClass('editable')){
				if((posi.top+$selectBoard.height()) > $(this).height()){
					selectBoardPos.top = selectBoardPos.top-$selectBoard.height()*2+$target.height();
				}
				// console.log("posi.left: "+ posi.left+ " selectBoard: " + $selectBoard.width() + " this: "+ $(this).width());
				if((posi.left+$selectBoard.width()*2) > $(this).width()){
					selectBoardPos.left = selectBoardPos.left-$selectBoard.width()*2+$target.width();
				}
