...
function check(i, j, x) {
  return checkNum(x) && checkRow(i, x) && checkColumn(j, x) && checkBlock(i, j, x);
}
...
function setTable(a) {
  for (var i = 0; i < 9; i++) {
    for (var j = 0; j < 9; j++) {
      if (a[i][j] !== 0) {
        table[i][j].innerHTML = '<span>' + a[i][j] + '</span>';
      } else {
        table[i][j].innerHTML = '<span><input type="text" maxlength="1" onchange="onInput(' + i + ',' + j + ');" onfocus="changecss(' + i + ',' + j + ');"/></span>';
      }
    }
  }
}
...
// 修改css
function changecss(i, j) {
  console.log("i: " + i + " j: " + j);
  // $(".sudoku_wrap li:nth-child(3n) span").addClass("common");
  // 清除高亮
  for (let k = 0; k < 9; k++) {
    for (let l = 0; l < 9; l++) {
    let num = k*9+l+1
    // console.log("i: " + i + " j: " + j + " k*9+j+1: " + num);
    let cssstr = ".sudoku_wrap li:nth-child(" + num + ") span";
    $(cssstr).removeClass("common");
    }
  }
  // 列高亮
  for (let k = 0; k < 9; k++) {
...
function onInput(i, j) {
  var inputElement = table[i][j].firstElementChild.firstElementChild;
...
function gameOver() {
  endTimer();
  var restart = confirm('恭喜完成游戏，点击重新开始');
  if(restart) {
    startGame();
  }
}
...
function showAnswer() {
  for (var i = 0; i < 9; i++) {
    for (var j = 0; j < 9; j++) {
      if (sudoku[i][j] !== 0) {
        table[i][j].innerHTML = '<span>' + answer[i][j] + '</span>';
      } else {
        table[i][j].innerHTML = '<span class="answer"> ' + answer[i][j] + '</span>';
      }
    }
  }
  endTimer();
